...
## 一句话总结
> HashMap基于数组+链表/红黑树实现，通过hashCode定位桶、equals确认key，1.8引入红黑树优化链表过长问题。

## 八股速记

**Q: HashMap ⭐ B**

- 数组 + 链表/红黑树（JDK8 链表长度 > 8 且容量 ≥ 64 转红黑树）；线程不安全，并发用 **ConcurrentHashMap**（分段/CAS+synchronized）。
- **HashMap vs HashTable**：后者线程安全但全表锁、已过时。