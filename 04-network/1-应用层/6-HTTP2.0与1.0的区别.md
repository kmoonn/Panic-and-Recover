...
- **1.0**：每请求一个 TCP 连接，用完即关。
- **1.1**：默认**长连接**（Keepalive）、管线化（少用）、支持分块传输、Host 头。存在**队头阻塞**（一个响应慢卡住后面）。
- **2**：**二进制分帧**、**多路复用**（一个连接并发多请求，解决 HTTP 层队头阻塞）、头部压缩（HPACK）、服务端推送。但底层仍是 TCP，**TCP 层队头阻塞**还在。
- **3**：底层换成 **QUIC（基于 UDP）**，彻底解决队头阻塞、连接迁移、握手更快（0-RTT/1-RTT）。
