...
**问：Kafka vs RocketMQ vs RabbitMQ（含 Kafka 使用与原理）⭐ B**

> ⚠️ 简历中间件行写"熟悉 Kafka、RabbitMQ"——要到"会用 + 懂核心原理"层级：能说清收发消息的基本模型 + Kafka 为什么高吞吐。以 **Kafka** 为主线讲（最常考），其余对比记特色。
...
|---|---|---|---|
| 定位 | 高吞吐日志/流 | 业务消息（阿里） | 通用消息（AMQP） |
| 吞吐 | 极高 | 高 | 中 |
| 顺序 | 分区内有序 | 支持顺序消息 | 一般 |
| 特色 | 大数据生态 | 事务消息/延迟消息 | 路由灵活、协议丰富 |

**② Kafka 核心概念 + 怎么用（会用即可）**
- **模型**：Producer 把消息发到某个 **Topic（主题）**；Topic 分成多个 **Partition（分区）**；Consumer 按 **Consumer Group（消费组）** 订阅拉取。
  ```
  Producer → Topic(分成 Partition0/1/2…) → Consumer Group(组内每个消费者认领若干分区)
  ```
- **发送**：指定 topic + 可选 key，`producer.send(topic, key, value)`；**key 决定进哪个分区**（同 key → 同分区 → 保证该 key 顺序）。
- **消费**：消费者**主动 pull**（不是 broker push），处理完提交 **offset**（消费位点）；重启后从上次 offset 续着消费。
- **消费组语义**：同一分区**只会被组内一个消费者**消费（保证不重复分摊）；组内消费者数 > 分区数则多出的空闲——**分区数决定并发上限**。不同组各自独立消费全量（广播效果）。

**③ Kafka 为什么高吞吐（原理，高频追问）**
- **分区并行**：一个 Topic 多分区分散到多 broker，读写天然水平并行。
- **顺序写磁盘 + Page Cache**：消息**追加写（append-only）**到日志文件，顺序 IO 比随机 IO 快几个数量级；读写走操作系统页缓存。
- **零拷贝（sendfile）**：数据从磁盘经内核直接送网卡，不在用户态来回拷贝（呼应 `03-os-linux.md` 零拷贝）。
- **批量 + 压缩**：Producer 攒批发送、可压缩，摊薄网络/IO 开销。

**⭐ 加分/易错**：
- 日志采集/大数据用 **Kafka**；电商业务、要事务/延迟消息用 **RocketMQ**；中小系统灵活路由用 **RabbitMQ**。选型看**吞吐 + 功能 + 团队生态**。
- **Kafka 保证的是"分区内有序"，不是全局有序**——要顺序就用相同 key 路由到同一分区（呼应 Q7）。
- **offset 提交时机**决定语义：先提交后处理可能丢消息（at-most-once），先处理后提交可能重复（at-least-once）→ 所以要**消费端幂等**（呼应 Q7）。
- RabbitMQ 的核心是 **Exchange（交换机）按路由规则分发到 Queue**（direct/topic/fanout），和 Kafka 的分区模型不同，别混。
