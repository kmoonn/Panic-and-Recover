...
**Q: FastAPI：异步 / Pydantic / 依赖注入**

- **是什么**：Python 的**现代异步 Web 框架**，基于 **Starlette（异步）+ Pydantic（校验）**，主打高性能、类型提示驱动、自动生成 API 文档。
- **异步（核心优势）**：原生支持 `async/await`，基于 **ASGI**（不是传统 WSGI）——**IO 密集**（调数据库、调大模型、发 HTTP）场景下用协程高并发，不必一请求一线程（呼应 `01-language-testing.md` GIL：Python IO 密集用 asyncio）。
- **Pydantic 数据校验**：用 Python 类型注解定义请求/响应模型，FastAPI **自动做参数校验 + 序列化 + 报错**，不合法直接返回 422。这也是 RAG/大模型场景**逼 LLM 输出稳定 JSON** 的利器（呼应快手效能星盘 Pydantic 校验 `kuaishou/01-xingpan.md`）。
- **依赖注入（Depends）**：用 `Depends()` 声明依赖（如取当前用户、开 DB 会话、鉴权），框架自动注入、可复用可测试——思想和 Spring DI 相通（呼应 Q1）。
- **自动文档**：基于类型自动生成 **OpenAPI/Swagger** 交互文档，`/docs` 直接可调。

**⭐ 加分/易错**：
- **别说"FastAPI 天生比 Flask 快"**——快主要来自**异步 + Starlette**，同步阻塞代码写在 `async` 里反而拖垮 event loop（阻塞调用要丢线程池，正是 RAG 项目 `run_in_executor` 的用法，回 `ceec/01-rag-retrieval.md`）。
- **SSE 流式**：大模型问答的流式返回（逐 token 吐）常用 FastAPI 的 `StreamingResponse`——RAG 前端聊天页就是 SSE（回 `ceec/00-rag-platform.md`）。
- 测开视角：Pydantic 模型让**接口契约明确**，可据此自动生成参数化测试、边界用例（呼应文末测开视角）。
