...
CPython 同一时刻只有一个线程执行字节码 → **多线程不能利用多核做 CPU 密集**；IO 密集可以（IO 时释放 GIL）。CPU 密集用**多进程**绕过。

**⭐ 加分/易错**：这就是为什么 Python 高并发 IO 用 asyncio 协程 / 多线程，CPU 密集用 multiprocessing——呼应 RAG 项目 asyncio + 线程池的选择。
