...
- **IoC（控制反转）**：对象的创建和依赖管理**交给容器**，不再自己 `new`。是一种设计思想。
- **DI（依赖注入）**：IoC 的**实现方式**——容器把依赖对象"注入"进来。
- **注入方式**：构造器注入（推荐，可保证 final/不可变）、Setter 注入、字段注入（`@Autowired`，简单但不利于测试）。

**⭐ 加分/易错**：
- IoC 是思想、DI 是手段，别混为一谈。
- **构造器注入对测试友好**——不用启容器就能 `new` 出来手动传 mock 依赖，这点在写单测时很关键（呼应文末测开视角）。
- `@Autowired` 按类型注入，多个同类型 Bean 用 `@Qualifier` / `@Primary` 消歧。
