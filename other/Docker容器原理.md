...
**问：Docker：镜像 / 容器 / Dockerfile / 分层 ⭐⭐**

**答（要点式）**：
- **是什么**：Docker 是**操作系统级虚拟化**——把应用 + 依赖 + 运行环境打包成**镜像**，到哪都一样跑，解决"在我机器上是好的"。
- **镜像 vs 容器**：**镜像（image）= 只读模板**（类比"类"）；**容器（container）= 镜像跑起来的运行实例**（类比"对象"），容器在镜像上加一层**可写层**。
- **和虚拟机的区别**：VM 虚拟一整套硬件 + 独立 OS 内核，重；容器**共享宿主机内核**，靠 **Namespace（隔离视图：进程/网络/挂载）+ Cgroups（限制资源：CPU/内存）** 实现隔离，轻量、秒起。
- **Dockerfile**：镜像的构建脚本，常用指令 `FROM`（基础镜像）/`RUN`（执行命令）/`COPY`/`EXPOSE`/`CMD`（启动命令）。
- **分层存储**：每条指令生成一**只读层**，层间**缓存复用**——改一层只重建那层及之后的，构建快、镜像省空间。

**⭐ 加分/易错**：
- 别说"容器是轻量级虚拟机"——本质差异是**共享内核 vs 独立内核**。
- **镜像分层缓存**：把不变的依赖安装放前面、变动的代码 `COPY` 放后面，能最大化缓存命中、加速构建。
- 容器**无状态**思想：数据要持久化用 **Volume 卷挂载**，别写进容器可写层（容器销毁即丢）。
